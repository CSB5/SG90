...
```{r}
current_study_res <- read_tsv(here('results_0.1', 'species_associations_high_stringency.tsv')) %>% 
...
current_study_res <- current_study_res %>% filter(padj < 0.05)  %>% filter(!(org_list %in% remove_species))
```
