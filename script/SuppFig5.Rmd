...
```{r}
valid_res <- read_tsv(here('results', 'Validation_other_cohorts.tsv'))
...
high_sens_results <- read.table(here('results', 'high_sens_results_with_chinese.txt'),
                                header = T, sep = '\t')
...
valid_res_for_plot <- valid_res_for_plot %>% 
  left_join(high_sens_results %>% 
              dplyr::select(org_list, Validation, HighConf),
            by = 'org_list') %>% 
